"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }var _Recordjs = require('../models/Record.js'); var _Recordjs2 = _interopRequireDefault(_Recordjs);
...
class PixController{
        ...
        if (status === 'CONCLUIDA') {
            try {
                for (const update of updates) {
                    ...
                    const record = await _Recordjs2.default.update(
                        {
                            status: statusMK,
                    ...
                    const record_pppoe = await _Recordjs2.default.findOne({where: {id: update.idValor}});

                    const sis_cliente = await _Clientjs2.default.findOne({where: {login: record_pppoe.dataValues.login}})
                    ...
                    const sis_update = await _Clientjs2.default.update({observacao: "sim", rem_obs: dataFormatada}, {where: {login: sis_cliente.dataValues.login}});
    ...
    async gerarPixAberto(req, res){
        ...
        const cliente = await _Recordjs2.default.findOne({where: {login: pppoe, status: "aberto", datadel: null}, order: [['datavenc', 'ASC']],});
        ...
        const sis_cliente = await _Clientjs2.default.findOne({where: {login: pppoe, cpf_cnpj: cpf}});
    ...
    async gerarPix(req, res){
        ...
        const cliente = await _Recordjs2.default.findOne({where: {login: pppoe, status: "vencido", datadel: null}, order: [['datavenc', 'ASC']],});
    ...
    async gerarPixAll(req, res){
        ...
        const cliente = await _Recordjs2.default.findAll({
            where: {
