"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }var _Recordjs = require('../models/Record.js'); var _Recordjs2 = _interopRequireDefault(_Recordjs);
...
var _sdknodeapisefi = require('sdk-node-apis-efi'); var _sdknodeapisefi2 = _interopRequireDefault(_sdknodeapisefi);
var _dotenv = require('dotenv'); var _dotenv2 = _interopRequireDefault(_dotenv);
var _crypto = require('crypto'); var _crypto2 = _interopRequireDefault(_crypto);
var _whatsapp = require('whatsapp'); var _whatsapp2 = _interopRequireDefault(_whatsapp);
...
const logFilePath = _path2.default.join(__dirname, 'log.json');
const logMsgFilePath = _path2.default.join(__dirname, 'msg.json');
...
class WhatsPixController{
        ...
        if (body.entry && body.entry[0].changes) {
            ...
            if (changes && Array.isArray(changes) && changes.length > 0) {
                ...
                if(type === "interactive"){
                    ...
                    mensagemCorpo = _optionalChain([messages, 'optionalAccess', _ => _[0], 'optionalAccess', _2 => _2.text, 'optionalAccess', _3 => _3.body]);
                }
                ...
                if (mensagemCorpo || type) {
                    ...
                    _fs2.default.readFile(logMsgFilePath, 'utf8', (err, data) => {
                        let logs = [];
                        ...
                        _fs2.default.writeFile(logMsgFilePath, jsonString, 'utf8', (err) => {
                            if (err) {
            ...
            case 'awaiting_cpf':
                if(this.verificaType(type)){
                    if(texto.toLowerCase() === 'inicio' || texto.toLowerCase() === 'início'){
                        ...
                        const sis_cliente = await _Clientjs2.default.findAll({ attributes: ['id', 'nome', 'endereco', 'login', 'numero'], where: { cpf_cnpj: cpf, cli_ativado: 's' } });
    ...
    async enviarBoleto(pppoe, celular, end, cpf){
        const cliente = await _Recordjs2.default.findOne({where: {login: pppoe, status:{[_sequelize.Op.or]: ['vencido', 'aberto']}, datadel: null}, order: [['datavenc', 'ASC']],});

        const sis_cliente = await _Clientjs2.default.findOne({where: {login: pppoe, cpf_cnpj: cpf, cli_ativado: 's'}});
        ...
        const efipayLoc = new (0, _sdknodeapisefi2.default)(options)
        ...
        const efipayLocLink = new (0, _sdknodeapisefi2.default)(options);
       ...
       valor = valorFinalFormatado;
        ...
        _fs2.default.readFile(logFilePath, 'utf8', (err, data) => {
            let logs = [];
            if (err && err.code === 'ENOENT') {
                console.log('Arquivo de log não encontrado, criando um novo.');
            } else if (err) {
            ...
            _fs2.default.writeFile(logFilePath, jsonString, 'utf8', (err) => {
                if (err) {
                    console.error('Erro ao escrever no arquivo de log:', err);
                    return;
                }
        ...
        const efipay = new (0, _sdknodeapisefi2.default)(options);
        ...
        let params = {
            txid: _crypto2.default.randomBytes(16).toString('hex'),
        }
    ...
    async enviarMensagemVencimento(receivenumber, dia, linha_dig, pix, end, valor, pppoe, numero, boletoID){
        try{
            ...
            if(linha_dig !== null){
                await this.downloadPdfFromSftp(receivenumber, process.env.SFTP_HOST,process.env.SFTP_USER,process.env.SFTP_PASSWORD,`${process.env.PDF_PATH}${boletoID}.pdf`, _path2.default.join(__dirname, '..', '..', 'temp', `${boletoID}.pdf`));
    ...
    async downloadPdfFromSftp(receivenumber, host, username, password, remoteFilePath, localFilePath) {        
        const client = new (0, _ssh2sftpclient2.default)();
        try {
    ...
    async MensagemTermos(receivenumber, header, body, right_text_url, url_site){
        try {
            ...
            console.error('Erro ao enviar mensagem com botão de link:', _optionalChain([error, 'access', _4 => _4.response, 'optionalAccess', _5 => _5.data]) || error.message);
        }
    }
    ...
    try {
        ...
        console.error('Erro ao enviar mensagem de lista:', _optionalChain([error, 'access', _6 => _6.response, 'optionalAccess', _7 => _7.data]) || error.message);
    }
    }
    ...
    async MensagemBotao(receivenumber, texto, title1, title2 = 0, title3 = 0) {
        try {
            ...
            console.error('Error sending button message:', _optionalChain([error, 'access', _8 => _8.response, 'optionalAccess', _9 => _9.data]) || error.message);
        }
    }
    
    async getMediaID(receivenumber, filePath, type, messaging_product = "whatsapp") {
        const formData = new (0, _formdata2.default)();
        formData.append('file', _fs2.default.createReadStream(filePath)); 
        ...
        try {
            ...
            console.error('Erro ao enviar a mídia:', _optionalChain([error, 'access', _10 => _10.response, 'optionalAccess', _11 => _11.data]) || error.message);
        }
    }
    ...
    async MensagensDeMidia(receivenumber, type, mediaID, filename) {
        try {
            ...
            console.error('Error sending media message:', _optionalChain([error, 'access', _12 => _12.response, 'optionalAccess', _13 => _13.data]) || error.message);
        }
    }
    

}
exports. default = new WhatsPixController();